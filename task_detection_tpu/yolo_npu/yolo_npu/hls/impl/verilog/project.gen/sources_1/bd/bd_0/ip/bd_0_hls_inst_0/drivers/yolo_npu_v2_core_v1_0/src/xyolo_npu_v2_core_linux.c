...
typedef struct {
    ...
    XYolo_npu_v2_core_uio_map maps[ MAX_UIO_MAPS ];
} XYolo_npu_v2_core_uio_info;

/***************** Variable Definitions **************************************/
static XYolo_npu_v2_core_uio_info uio_info;
...
static int uio_info_read_name(XYolo_npu_v2_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XYolo_npu_v2_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XYolo_npu_v2_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XYolo_npu_v2_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XYolo_npu_v2_core_Initialize(XYolo_npu_v2_core *InstancePtr, const char* InstanceName) {
	XYolo_npu_v2_core_uio_info *InfoPtr = &uio_info;
