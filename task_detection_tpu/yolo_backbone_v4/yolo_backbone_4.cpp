...
#pragma HLS UNROLL
                        for (int r = 0; r < 3; r++) {
#pragma HLS UNROLL
                            for (int c = 0; c < 3; c++) acc += window[ic][r][c] * weights[oc][ic][r][c];
                        }
                    }
                    p_out.ch[oc] = acc;
                }
...
// ======================================================
// Splitter & Residual Cropper (4x4 -> 2x2)
// ======================================================
void split_and_crop(
    hls::stream<pixel_64_t>& in_stream, 
    hls::stream<pixel_64_t>& out_conv2, 
    hls::stream<pixel_64_t>& out_residual
) {
    for (int r = 0; r < L1_OUT_H; r++) {
        for (int c = 0; c < L1_OUT_W; c++) {
#pragma HLS PIPELINE II=1
            pixel_64_t p = in_stream.read();
            out_conv2.write(p); 
...
// ======================================================
// Layer 2: Conv2D (64 -> 64) | FOLDED TO 288 DSPs
// ======================================================
void conv_layer_2(
    hls::stream<pixel_64_t>& in_stream, 
...
#pragma HLS ARRAY_PARTITION variable=window complete dim=0
    ...
    for (int row = 0; row < L1_OUT_H; row++) {
        for (int col = 0; col < L1_OUT_W; col++) {
            pixel_64_t p_in = in_stream.read();
...
#pragma HLS UNROLL
            ...
            if (row >= 2 && col >= 2) {
                ...
                for (int oc = 0; oc < CH_DIM; oc++) {
                    ...
                    // FOLDING MECHANISM: Process 64 channels in two halves (32 at a time)
                    for (int ic_block = 0; ic_block < 2; ic_block++) {
#pragma HLS PIPELINE II=1
                        for (int ic = 0; ic < 32; ic++) {
#pragma HLS UNROLL
                            int actual_ic = ic_block * 32 + ic;
                            for (int r = 0; r < 3; r++) {
#pragma HLS UNROLL
                                for (int c = 0; c < 3; c++) {
#pragma HLS UNROLL
                                    acc += window[actual_ic][r][c] * weights[oc][actual_ic][r][c];
                                }
                            }
                        }
                    }
...
// ======================================================
// Residual Add, Leaky ReLU, and MaxPool 
// ======================================================
void residual_add_relu(
    hls::stream<pixel_64_t>& conv_stream, 
    hls::stream<pixel_64_t>& res_stream, 
    hls::stream<pixel_64_t>& out_stream
) {
    for (int i = 0; i < L2_OUT_H * L2_OUT_W; i++) {
#pragma HLS PIPELINE II=1
        pixel_64_t p_conv = conv_stream.read();
        pixel_64_t p_res = res_stream.read();
        pixel_64_t p_out;
...
void max_pool_2x2(hls::stream<pixel_64_t>& in_stream, hls::stream<pixel_64_t>& out_stream) {
...
#pragma HLS PIPELINE II=1
            ...
            if (c % 2 == 0) {
                for(int ch = 0; ch < CH_DIM; ch++) left_pixel[ch] = p_in.ch[ch];
            } else {
                data_t h_max[CH_DIM];
...
#pragma HLS UNROLL
                ...
                if (r % 2 == 0) {
                    for(int ch = 0; ch < CH_DIM; ch++) top_row_max[c / 2][ch] = h_max[ch];
                } else {
                    pixel_64_t p_out;
                    for(int ch = 0; ch < CH_DIM; ch++) {
#pragma HLS UNROLL
...
// ======================================================
// Memory I/O & Top-Level Shell
// ======================================================
void load_image(data_t* image_in, hls::stream<pixel_32_t>& stream_in) {
...
void store_image(hls::stream<pixel_64_t>& stream_out, data_t* image_out) {
    for(int i = 0; i < POOL_OUT_H * POOL_OUT_W; i++) {
#pragma HLS PIPELINE II=1
        pixel_64_t p = stream_out.read();
...
// Notice dim=2 is cyclic factor 32. This aligns perfectly with the ic_block folding!
#pragma HLS ARRAY_PARTITION variable=l2_w cyclic factor=32 dim=2
#pragma HLS ARRAY_PARTITION variable=l2_w complete dim=3
#pragma HLS ARRAY_PARTITION variable=l2_w complete dim=4

    data_t l2_b[CH_DIM]; 

    // FLATTENED AXI LOADS (Guarantees II=1 Burst)
    for(int oc=0; oc<CH_DIM; oc++) {
#pragma HLS PIPELINE II=1
        l1_b[oc] = l1_b_in[oc];
    }
...
#pragma HLS DATAFLOW
    hls::stream<pixel_32_t> s_img_in("s_img_in");
    hls::stream<pixel_64_t> s_l1_out("s_l1_out");
    hls::stream<pixel_64_t> s_conv2_in("s_conv2_in");
    
    hls::stream<pixel_64_t> s_res_sync("s_res_sync");
#pragma HLS STREAM variable=s_res_sync depth=8 // Smaller depth since dimensions are tiny now

    hls::stream<pixel_64_t> s_l2_out("s_l2_out");
    hls::stream<pixel_64_t> s_add_out("s_add_out");
    hls::stream<pixel_64_t> s_pool_out("s_pool_out");
