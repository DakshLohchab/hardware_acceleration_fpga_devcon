...
typedef struct {
    ...
    XYolo_backbone_v4_uio_map maps[ MAX_UIO_MAPS ];
} XYolo_backbone_v4_uio_info;

/***************** Variable Definitions **************************************/
static XYolo_backbone_v4_uio_info uio_info;
...
static int uio_info_read_name(XYolo_backbone_v4_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XYolo_backbone_v4_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XYolo_backbone_v4_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XYolo_backbone_v4_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XYolo_backbone_v4_Initialize(XYolo_backbone_v4 *InstancePtr, const char* InstanceName) {
	XYolo_backbone_v4_uio_info *InfoPtr = &uio_info;
