...
// ======================================================
// 1. Layer 1: Conv2D (3 -> 16) | Unrolled (432 DSPs)
// ======================================================
void conv_layer_1(
...
// ======================================================
void leaky_relu_1(hls::stream<pixel_l1_out_t>& in_stream, hls::stream<pixel_l1_out_t>& out_stream) {
...
#pragma HLS UNROLL
            p_out.ch[c] = (p_in.ch[c] > 0) ? p_in.ch[c] : (data_t)(p_in.ch[c] * (data_t)0.1f);
        }
        out_stream.write(p_out);
    }
}
...
// ======================================================
// 3. Layer 2: Conv2D (16 -> 32) | Folded (288 DSPs)
// ======================================================
void conv_layer_2(
...
// ======================================================
// 4. Max Pooling: 2x2, Stride 2 (0 DSPs)
// ======================================================
void max_pool_2x2(hls::stream<pixel_l2_out_t>& in_stream, hls::stream<pixel_l2_out_t>& out_stream) {
    // Cache the maximums of the top row pairs
    data_t top_row_max[POOL_OUT_W][L2_OUT_CH];
#pragma HLS ARRAY_PARTITION variable=top_row_max complete dim=2

    for (int r = 0; r < L2_OUT_H; r++) {
        data_t left_pixel[L2_OUT_CH];
#pragma HLS ARRAY_PARTITION variable=left_pixel complete dim=1

        for (int c = 0; c < L2_OUT_W; c++) {
#pragma HLS PIPELINE II=1
            pixel_l2_out_t p_in = in_stream.read();

            if (c % 2 == 0) {
                // Save left pixel in register
                for(int ch = 0; ch < L2_OUT_CH; ch++) {
#pragma HLS UNROLL
                    left_pixel[ch] = p_in.ch[ch];
                }
            } else {
                // Compute horizontal max for the current row
                data_t h_max[L2_OUT_CH];
                for(int ch = 0; ch < L2_OUT_CH; ch++) {
#pragma HLS UNROLL
                ...
                if (r % 2 == 0) {
                    // Even row: store horizontal max in the line buffer
                    for(int ch = 0; ch < L2_OUT_CH; ch++) {
#pragma HLS UNROLL
                        top_row_max[c / 2][ch] = h_max[ch];
                    }
                } else {
                    // Odd row: compute vertical max with top_row_max and output!
                    pixel_l2_out_t p_out;
                    for(int ch = 0; ch < L2_OUT_CH; ch++) {
#pragma HLS UNROLL
...
// ======================================================
// 5. Memory I/O & Top-Level Native Shell
// ======================================================
void load_image(data_t* image_in, hls::stream<pixel_l1_in_t>& stream_in) {
...
void store_image(hls::stream<pixel_l2_out_t>& stream_out, data_t* image_out) {
    for(int i = 0; i < POOL_OUT_H * POOL_OUT_W; i++) {
#pragma HLS PIPELINE II=1
        pixel_l2_out_t p = stream_out.read();
        for(int c = 0; c < L2_OUT_CH; c++) image_out[i * L2_OUT_CH + c] = p.ch[c];
    }
}
...
#pragma HLS DATAFLOW
    ...
    hls::stream<pixel_l2_out_t> stream_pool_out("stream_pool_out");
    ...
    max_pool_2x2(stream_l2_out, stream_pool_out);
    store_image(stream_pool_out, image_out);
}
...
#pragma HLS ARRAY_PARTITION variable=l2_b cyclic factor=2 dim=1
    ...
    for(int oc=0; oc<L1_OUT_CH; oc++) {
#pragma HLS PIPELINE II=1
        l1_b[oc] = l1_bias_in[oc];
        for(int ic=0; ic<L1_IN_CH; ic++) {
            for(int r=0; r<3; r++) {
                for(int c=0; c<3; c++) {
                    int idx = oc*(L1_IN_CH*9) + ic*9 + r*3 + c;
                    l1_w[oc][ic][r][c] = l1_weights_in[idx];
                }
            }
        }
    }
...
#pragma HLS PIPELINE II=1
        l2_b[oc] = l2_bias_in[oc];
        for(int ic=0; ic<L1_OUT_CH; ic++) {
            for(int r=0; r<3; r++) {
                for(int c=0; c<3; c++) {
                    int idx = oc*(L1_OUT_CH*9) + ic*9 + r*3 + c;
                    l2_w[oc][ic][r][c] = l2_weights_in[idx];
                }
            }
        }
    }
